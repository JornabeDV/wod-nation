...
export async function generateBracketAction(competitionId: string, categoryId: string) {
  ...
  const allMatches: { round: number; position: number; athlete1Id?: string; athlete2Id?: string }[] = [];
  ...
  revalidatePath(`/dashboard/competitions/${competitionId}/brackets`);
  return { success: true };
}

export async function getBracketMatchesAction(competitionId: string, categoryId: string) {
...
export async function updateBracketMatchAction(
  matchId: string,
  ...
  revalidatePath(`/dashboard/competitions/${match.competitionId}/brackets`);
  return match;
}
